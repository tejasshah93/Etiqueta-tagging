...
namespace Integrate
{
    public sealed partial class ImagingProperties : LayoutAwarePage
    {
        ...
        private void tagbutton_Click(object sender, RoutedEventArgs e)
        {
            ...
            Button[] Buttons = new Button[keywordsArray.Length];
            for (int i = 0; i < keywordsArray.Length - 1; i++)
            {
                Buttons[i] = new Button();
                Buttons[i].Click += ImagingProperties_Click;

                Buttons[i].Tag = keywordsArray[i].TrimStart();
               
                Buttons[i].Content = keywordsArray[i].TrimStart();
                Buttons[i].Foreground = new SolidColorBrush(Colors.Black);
                tagpanel.Children.Add(Buttons[i]);
        ...
        void ImagingProperties_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                Button temp = (Button)sender;
                string data = (string)temp.Tag;             
                Button x = new Button();
                x.Content = "clicked!!";
                tagpanel.Children.Add(x);
                this.Frame.Navigate(typeof(Search),data);
            }
            catch (Exception ee)
            {
                title.Text = ee.Message;
            }
        }
